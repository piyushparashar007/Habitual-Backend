            ...
            def callback(ch, method, properties, body):
                try:
                    ...
                    if data.get("event") == "ACTIVITY_LOGGED":
                        ...
                        conn.commit()
                        conn.close()
                        print(f"Recommendations: Logged {payload['id']}")
                except Exception as e:
                    print(f"Error processing message: {e}")

            channel.basic_consume(queue=queue_name, on_message_callback=callback, auto_ack=True)
            print("Recommendations Service: Waiting for activity_events in RabbitMQ...")
            channel.start_consuming()
        except pika.exceptions.AMQPConnectionError:
            print("Recommendations Service: Connection to RabbitMQ failed, retrying in 5s...")
            time.sleep(5)
        except Exception as e:
            print(f"Recommendations Service: Unexpected error: {e}")
            time.sleep(5)
...
def compute_recommendations(req: ComputeRequest):
    ...
    morning_logs = [l for l in logs if 5 <= datetime.fromisoformat(l[0].replace('Z', '')).hour < 12]
    evening_logs = [l for l in logs if 17 <= datetime.fromisoformat(l[0].replace('Z', '')).hour < 23]
    
    morning_success = sum(1 for l in morning_logs if l[1] == 'done') / len(morning_logs) if morning_logs else 0
    evening_success = sum(1 for l in evening_logs if l[1] == 'done') / len(evening_logs) if evening_logs else 0
    
    suggestion = "Keep up your consistency!"
    if morning_success > evening_success + 0.2:
        suggestion = "You are 20%+ more consistent in the morning. Consider scheduling this habit earlier."
    elif evening_success > morning_success + 0.2:
    ...
    cursor.execute('INSERT INTO recommendations (id, user_id, activity_id, suggestion, type, created_at) VALUES (?,?,?,?,?,?)',
                   (rec_id, req.user_id, req.activity_id, suggestion, "timing", datetime.now().isoformat()))
    conn.commit()
    conn.close()
...
def get_stats(user_id: str):
    ...
    # Success by hour (0-23)
    hourly = {i: {"done": 0, "total": 0} for i in range(24)}
    # Success by day of week (0=Mon, 6=Sun)
    daily = {i: {"done": 0, "total": 0} for i in range(7)}
    
    for created_at, status in logs:
        try:
